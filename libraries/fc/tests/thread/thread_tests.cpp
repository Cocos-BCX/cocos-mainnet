...
//#include <fc/thread/threadpool.hpp>  
...
BOOST_AUTO_TEST_CASE(executes_task)
{
    bool called = false;
    fc::thread thread("my");
    thread.async([&called]{called = true;
    
    
    
    }).wait();
     printf("this is da father!!\r\n");
    BOOST_CHECK(called);
}
...
{
    fc::thread thread("my");
    auto temp=thread.async([]{return (int)10;}).wait();
    BOOST_CHECK_EQUAL(10,temp);
}
...
BOOST_AUTO_TEST_CASE(executes_multiple_tasks)
{
    ...
    auto future2 = thread2.async([&called2]{called2 = true;int i=10; while(i--)
                                                                    {
                                                                        fc::yield();
                                                                        sleep(1);
                                                                        printf("future2:%d\r\n",i);
