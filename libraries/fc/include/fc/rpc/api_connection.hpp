...
namespace fc {
   ...
   namespace detail {
      ...
      class callback_functor                    //nico rpc call_back订阅返回函数构建器  _subscribe_callback, _pending_trx_callback, _block_applied_callback
      {
         public:
      ...
      {
         return [=]( const variants& args ) { 
            call_generic( f, args.begin(), args.end() );   // API 多参数处理
            return variant();
         };
      }
   ...
   class generic_api
   {
      public:
         ...
         variant call( uint32_t method_id, const variants& args )
         {
            ...
            //idump((method_id));
            return this->_methods[method_id](args);   // 此处将处理参数变形
         }
      ...
      private:
         ...
         R call_generic( const std::function<R(Arg0,Args...)>& f, variants::const_iterator a0, variants::const_iterator e )
         {
            ...
            return  call_generic<R,Args...>( 
                                this->bind_first_arg<R,Arg0,Args...>( f, a0->as< typename std::decay<Arg0>::type >() ),
                                 a0+1, e );
         }
         ...
         struct api_visitor
         {
            ...
            void operator()( const char* name, std::function<Result(Args...)>& memb )const {
               _api._methods.emplace_back( to_generic( memb ) );        //  各种不同类型的函数将被 to_generic() 统一转化成 \
                                                                            std::function<variant(const variants&)> 类型函数(单一参数 vector<variant> )
               _api._by_name[name] = _api._methods.size() - 1;          //  对应API类型下的所有被反射的函数将在此被展开注册
              // wdump(("nico debug")(name));                                       
            }
   ...
   class api_connection : public std::enable_shared_from_this<fc::api_connection>
   {
      public:
         ...
         variant receive_call( api_id_type api_id, const string& method_name, const variants& args = variants() )const  //  websocket 将使用此方法 
         {
         ...
         api_id_type register_api( const Interface& a )
         {
            ...
            if( itr != _handle_to_id.end() ) return itr->second;   //   ： 如果 已经注册则直接返回对应的API id 
            ...
            return _local_apis.size() - 1;  //  api append到末尾，返回末尾对应的 api id
         }
      ...
      private:
         std::vector< std::unique_ptr<generic_api> >             _local_apis;    // 通用型api 
         ...
         struct api_visitor
         {
            ...
            void operator()( const char* name, std::function<Result(Args...)>& memb )const 
            {
                ...
                memb = [con,api_id,name]( Args... args ) {
                    auto var_result = con->send_call( api_id, name, { 
                        convert_callbacks(con,args)...// 此处将args全部展开，展开方式：\
                                                    convert_callbacks(con,args1)，convert_callbacks(con,args2)...convert_callbacks(con,argsn)\
                                                    为send_call组装参数数组
                    } );
   ...
   {
      //wdump(("typeid(Api).name")(typeid(Api).name()));
      boost::any_cast<const Api&>(a)->visit( api_visitor( *this, c ) ); //nico 通用api访问者
   }
